...
imagePath = "/home/ccpcpp/Dropbox/code/darkest/resources/images/CoinsB.png"
...
if __name__ == "__main__":
  ...
  # ### YOUR CODE HERE
  # ###
  _, thresholdedImage = cv.threshold(imageB, 110, 255, cv.THRESH_BINARY)
  ...
  thresholdedImage = cv.morphologyEx(
    thresholdedImage,
    cv.MORPH_OPEN,
    cv.getStructuringElement(cv.MORPH_ELLIPSE, (32,32)),
    iterations=7
  )
  plt.imshow(thresholdedImage)
  ...
  thresholdedImage = cv.morphologyEx(
    thresholdedImage,
    cv.MORPH_CLOSE,
    cv.getStructuringElement(cv.MORPH_ELLIPSE, (72,72)),
    iterations=4
  )
  plt.imshow(thresholdedImage)
  plt.title("Eroded Image2")
  ...
  for idx, cnt in enumerate(contours[:-1]):
    ...
    cv.drawMarker(image, (x,y), (0, 0, 255), cv.MARKER_CROSS, 32, 2, cv.LINE_AA)
    # Fit an ellipse
    if len(cnt) < 5: continue
    ellipse = cv.fitEllipse(cnt)
    cv.ellipse(image, ellipse, (255,0,125), 2, cv.LINE_AA)
    cv.putText(image, str(idx+1), (x+40, y-40), cv.FONT_HERSHEY_SIMPLEX, 5, (0, 0, 255), 15)
