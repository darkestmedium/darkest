...
int main() {
  ...
  ///
  Mat channels[3];
  split(imageCopy, channels);
  ///
  /// YOUR CODE HERE
  ///
  Mat thresholdedImage = imageGray.clone();
  threshold(channels[1], thresholdedImage, 40, 255, THRESH_BINARY);
  ///
  /// YOUR CODE HERE
  ///
  ...
  Mat imageEroded;
  erode(thresholdedImage, imageEroded, kernel2);
  erode(thresholdedImage, imageEroded, kernel1);
  ///
  /// YOUR CODE HERE
  ///
  Mat imageDilated;
  dilate(imageEroded, imageDilated, kernel2);
  ...
  for (KeyPoint& keypoint : keypoints) {
    ...
    drawMarker(imageCopy, keypoint.pt, Scalar(0, 255, 0), MARKER_CROSS, keypoint.size*0.25, 1);
  }
